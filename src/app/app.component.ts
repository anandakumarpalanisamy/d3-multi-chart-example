...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  constructor() {
    this.margin = { top: 20, right: 50, bottom: 80, left: 60 };
  ...
  ngOnInit() {
    const self = this;
    const extentData = this.extentData;
    const hoursData = this.hoursData;
    this.data.forEach(function(d) {
      d.actualDate = d3.time.format('%Y-%m-%d').parse(d.actualDate);
      d.in = d3.time.format('%Y-%m-%dT%H:%M:%S').parse(d.in);
      d.out = d3.time.format('%Y-%m-%dT%H:%M:%S').parse(d.out);      
      extentData.push(d.in);
      extentData.push(d.out);
      hoursData.push(d.totalHours);
      hoursData.push(d.totalProductiveHours);
    });
  }
  ...
  ngAfterViewInit() {
    ...
    const valueline1 = d3.svg.line()
      .x(function (d) { return x(d.actualDate); })
      .y(function (d) { return y1(d.in); });

    const valueline2 = d3.svg.line()
      .x(function (d) { return x(d.actualDate); })
      .y(function (d) { return y1(d.out); });
    ...
    x.domain(d3.extent(this.data, function(d) { return d.actualDate; }));
    x0.domain(this.hoursKey.map(function (d) { return d; }));

    y1.domain(d3.extent(this.extentData, function(d) { return d; }));
    ...
    .enter().append('rect')
      .style('fill', '#2d45fc')
      .attr('x', function(d) { return x(d.actualDate); })
      .attr('width', 10)
    ...
    const lineLegend = svg.append('g')
      ...
      .attr('text-anchor', 'end')
        ...
        .enter().append('g')
          .attr('transform', function(d, i) { return 'translate(0,' + (i + 3) * 15 + ')'; });
